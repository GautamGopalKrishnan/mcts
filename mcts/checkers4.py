...
class CheckersEnv4:
    ...
    def step(self, action):
        ...
        if action[1][0]==0 or action[1][0]==3:
    ...
    def badiag(self, row, col):
        if self.board[row,col]!=1:
            if row>0 and col<3 and self.board[row-1,col+1]==0:
                return ((row,col),(row-1,col+1))
            if row>1 and col<2 and self.board[row-2,col+2]==0 and np.sign(self.board[row-1,col+1])==(-1)*np.sign(self.board[row,col]):
                return ((row,col),(row-2,col+2))
        return None
    
    def fdiag(self, row, col):
        if self.board[row,col]!=-1:
            if row<3 and col<3 and self.board[row+1,col+1]==0:
                return ((row,col),(row+1,col+1))
            if row<2 and col<2 and self.board[row+2,col+2]==0 and np.sign(self.board[row+1,col+1])==(-1)*np.sign(self.board[row,col]):
                return ((row,col),(row+2,col+2))
        return None
    
    def fadiag(self, row, col):
        if self.board[row,col]!=-1:
            if row<3 and col>0 and self.board[row+1,col-1]==0:
                return ((row,col),(row+1,col-1))
            if row<2 and col>1 and self.board[row+2,col-2]==0 and np.sign(self.board[row+1,col-1])==(-1)*np.sign(self.board[row,col]):
                return ((row,col),(row+2,col-2))
        return None
...
class CheckersGUI4:
    ...
    def __init__(self, window):
        ...
        self.action_buttons = [Button(self.window, Point(50+j*45,100+i*45), 45, 45, "({},{})".format(i,j)) for i in range(4) for j in range(4)]
        ...
        self.board = CheckersBoard4(self.window, Point(200, 250))

    def show_start(self):
        for b in self.action_buttons:
            b.undraw()
        self.board.undraw()
        self.banner.setText("Checkers4")
    ...
    def get_action1(self, actions):
        ...
        for i in range(4):
            for j in range(4):
                self.action_buttons[4*i+j].deactivate()
        for r in actions:
            self.action_buttons[4*r[0][0]+r[0][1]].activate()                
    ...
    def get_action2(self, actions, firstaction):
        ...
        for r in range(len(actions)):
            if actions[r][0][0]==firstaction[0] and actions[r][0][1]==firstaction[1]:
                self.action_buttons[4*actions[r][1][0]+actions[r][1][1]].activate()                
        #self.action_buttons[-1].activate()
        #self.action_buttons[-2].activate()
        while True:
            p = self.window.getMouse()
